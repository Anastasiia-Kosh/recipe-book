...
const Profile = async () => {
  ...
  const { totalRecipes } = await fetchMyRecipes();
  const { totalSavedRecipes } = await fetchSavedRecipes();
        ...
        <div className={css.profileNavigation}>
          <Link href="/my-recipes" className={css.navigationCard}>
            <div>
              ...
              <p className={css.cardDescription}>Рецепти, які ви створили</p>
            </div>

            <div className={css.cardFooter}>
              <span className={css.count}>{totalRecipes}</span>
          ...
          <Link href="/favorites" className={css.navigationCard}>
            ...
            <div className={css.cardFooter}>
              <span className={css.count}>{totalSavedRecipes}</span>
              <span className={css.cardLink}>
                Переглянути
                <Icon name="arrow-right" size={18} />
              </span>
